/*
 * Copyright (C) 2012 Manav Gautama <bandwidthcrunch@gmail.com>
 *
...
int board_init(void)
{
	gd->bd->bi_arch_number = MACH_TYPE_DARLO;
...
int board_eth_init(bd_t *bis)
{
	...
	int i;
	struct davinci_gpio *gpio4_base =
			(struct davinci_gpio *)DAVINCI_GPIO_BANK45;

	/* Configure PINMUX 3 to enable EMAC pins */
	writel((readl(PINMUX3) | 0x1affff), PINMUX3);

	/* Configure GPIO68 as output */
	writel((readl(&gpio4_base->dir) & ~(1 << 68)), &gpio4_base->dir);
	
	/* Toggle GPIO 68 */ /* Some hack to get sensor working */
	for (i = 0; i < 20; i++) {
		/* GPIO 68 low */
		writel((readl(&gpio4_base->out_data) & ~(1 << 68)),
						&gpio4_base->out_data);
